...
public class GridMesh {
    ...
    public GridMesh(Mesh mesh) {
        ...
        // 包围盒（BoundingBox 由顶点 min/max 计算，getMin/getMax 反推有 ULP 舍入，
        // 由 getAreaPosition 的 EPS 容忍）
    ...
    public ArrayList<Integer>[][] getFaceList() {
        ...
        if (area == null) {
            ...
            for (int i = 0; i < fCount; i++) {
                ...
                // 下面要看看三角形落在那几个area中。
                Vector2f p1 = getAreaPosition(boundsMinX, boundsMinY);
                Vector2f p2 = getAreaPosition(boundsMaxX, boundsMaxY);

                // 遍历附近的每个矩形，看看和这个三角形是否相交。
                for (int py = (int) p1.y; py <= (int) p2.y; py++) {
                    for (int px = (int) p1.x; px <= (int) p2.x; px++) {
                         ...
                         * 若矩形的任意一点落在三角形内，他们相交。
                         */
                        if (((int) p[0].x == px && (int) p[0].y == py) || ((int) p[1].x == px && (int) p[1].y == py)
                                || ((int) p[2].x == px && (int) p[2].y == py) || intersect(tri[0], tri[1], tri[2], box)
                                || intersect(tri, box)) {
     ...
     * @return
     */
    public Vector2f getAreaPosition(final float x, final float z) {
        // 不在此网格的边界内（epsilon 容忍 float 舍入：BoundingBox 以 center/extent 存储，
        // getMin/getMax 反推与顶点实际 min/max 有 ULP 级差异，如 0.0078）
        if (x < min.x - EPS || z < min.z - EPS || x > max.x + EPS || z > max.z + EPS) {
            return null;
        }
        ...
        return new Vector2f((int) col, (int) row);
    }
    ...
    public ArrayList<Integer> getFaceList(final Vector3f position) {
        Vector2f p = getAreaPosition(position.x, position.z);

        if (p != null) {
            return getFaceList((int) p.x, (int) p.y);
        } else {
            return null;
        }
    }
